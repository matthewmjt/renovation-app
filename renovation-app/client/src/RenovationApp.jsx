              ...
              {[...labourQuotes].sort((a, b) => Number(a.amount) - Number(b.amount)).map((q, i) => (
                <div key={q.id} style={{ border: q.confirmed ? "2px solid #1A1A1A" : "1px solid #EEEBE6", borderRadius: 10, padding: "12px 14px", marginBottom: 8, background: q.confirmed ? "#FAFAF8" : "white", display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <input value={q.contractorName}
                      onChange={e => { const next = labourQuotes.map(x => x.id === q.id ? { ...x, contractorName: e.target.value } : x); setLabourQuotes(next); commit({ lq: next }); }}
                      style={{ border: "none", outline: "none", fontWeight: 600, fontSize: 13, width: "100%", background: "transparent", color: "#1A1A1A" }} />
                    <input value={q.note || ""}
                      onChange={e => { const next = labourQuotes.map(x => x.id === q.id ? { ...x, note: e.target.value } : x); setLabourQuotes(next); commit({ lq: next }); }}
                      placeholder="Add a note…"
                      style={{ border: "none", outline: "none", fontSize: 11, width: "100%", background: "transparent", color: "#888", marginTop: 2 }} />
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 4, flexShrink: 0 }}>
                    <span style={{ fontSize: 14, color: "#555" }}>{"£"}</span>
                    <input type="number" min="0" step="0.01" value={q.amount}
                      onChange={e => { const next = labourQuotes.map(x => x.id === q.id ? { ...x, amount: Number(e.target.value) || 0 } : x); setLabourQuotes(next); commit({ lq: next }); }}
                      style={{ width: 80, fontSize: 18, fontWeight: 700, border: "none", outline: "none", background: "transparent", color: "#1A1A1A", textAlign: "right" }} />
                  </div>
...
// ─── Main App ─────────────────────────────────────────────────────────────────
// ─── Floor Plan Viewer ────────────────────────────────────────────────────────
function FloorPlanViewer({ lightbox, onClose }) {
      ...
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, zIndex: 10, display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 20px", background: "linear-gradient(rgba(0,0,0,0.5),transparent)", pointerEvents: "none" }}>
        ...
        <div style={{ display: "flex", gap: 8, pointerEvents: "all" }}>
          <button onClick={resetView}
            style={{ background: "rgba(255,255,255,0.15)", border: "none", color: "white", fontSize: 12, padding: "6px 12px", borderRadius: 8, cursor: "pointer", fontWeight: 500 }}>
            Reset
          </button>
          <button onClick={onClose}
            style={{ background: "rgba(255,255,255,0.15)", border: "none", color: "white", fontSize: 20, width: 36, height: 36, borderRadius: 8, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", lineHeight: 1 }}>
            {"×"}
          </button>
        </div>
      </div>

      <div style={{ position: "absolute", bottom: 20, left: "50%", transform: "translateX(-50%)", zIndex: 10, display: "flex", alignItems: "center", gap: 8, background: "rgba(0,0,0,0.5)", borderRadius: 10, padding: "6px 12px" }}>
        <button onClick={e => { e.stopPropagation(); setZoom(z => Math.max(z / 1.3, 0.25)); }}
          style={{ background: "rgba(255,255,255,0.15)", border: "none", color: "white", fontSize: 18, width: 30, height: 30, borderRadius: 6, cursor: "pointer", lineHeight: 1 }}>{"−"}</button>
        <span style={{ color: "white", fontSize: 12, minWidth: 40, textAlign: "center" }}>{Math.round(zoom * 100)}%</span>
        <button onClick={e => { e.stopPropagation(); setZoom(z => Math.min(z * 1.3, 8)); }}
          style={{ background: "rgba(255,255,255,0.15)", border: "none", color: "white", fontSize: 18, width: 30, height: 30, borderRadius: 6, cursor: "pointer", lineHeight: 1 }}>{"+"}</button>
      </div>

      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", cursor: dragging ? "grabbing" : zoom > 1 ? "grab" : "default" }}>
        <img src={lightbox.image} alt={lightbox.name} onMouseDown={onMouseDown} draggable={false}
          style={{ maxWidth: "92vw", maxHeight: "88vh", objectFit: "contain",
            transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoom})`,
            transformOrigin: "center center",
            transition: dragging ? "none" : "transform 0.1s ease",
            userSelect: "none", boxShadow: "0 8px 40px rgba(0,0,0,0.6)" }} />
      </div>

      {zoom === 1 && pan.x === 0 && pan.y === 0 && (
        <div style={{ position: "absolute", bottom: 64, left: "50%", transform: "translateX(-50%)", color: "rgba(255,255,255,0.35)", fontSize: 11, whiteSpace: "nowrap", pointerEvents: "none" }}>
...
export default function RenovationApp({ initialData, onSave }) {
  ...
  const saveEditProp = () => {
    ...
    setProps_(prev => prev.map(p => p.id === editPropData.id ? {
      ...
      rooms,
      floors: editPropData.floors || [],
      roomFloors: editPropData.roomFloors || {},
                            ...
                            <td style={{ padding: "10px 14px", textAlign: "right" }}>
                              <div style={{ display: "flex", gap: 6, justifyContent: "flex-end", alignItems: "center" }}>
                                <button onClick={() => setExpandedCostIdx(isExpanded ? null : i)}
                                  style={{ background: "none", border: "none", color: costDocs.length > 0 ? "#3730A3" : "#BBB", cursor: "pointer", fontSize: 12 }} title="Documents">{"⊕"}</button>
